    ...
    try {
      ...
      try {
        ...
        if (wsBaseUrl.includes('localhost') && !window.location.hostname.includes('localhost')) {
          ...
          // For production behind nginx, don't include port when using standard HTTPS
          const apiPort = window.location.port === '443' || window.location.port === '' ? '' : `:${window.location.port}`;
